...
$Customerno=$_REQUEST['invoice'];
 ...
 $row = odbc_exec($conn, "SELECT * FROM [Temp Student] WHERE [Company Name]='$ms' AND [Registration No_]='$Customerno'") or die(odbc_errormsg($conn));
 $crd_amt = odbc_exec($conn, "SELECT SUM([Credit Amount]) AS [Credit] FROM [Ledger Credit] WHERE [Reverse]=0 AND ([Customer No]='".odbc_result($row, "No_")."' OR [Customer No]='".odbc_result($row, "Registration No_")."')  " ) or die(odbc_errormsg($conn));
 $deb_amt = odbc_exec($conn, "SELECT SUM([Credit Amount]) AS [Debit] FROM [Ledger Debit] WHERE [Reverse]=0 AND ([Customer No]='".odbc_result($row, "No_")."' OR [Customer No]='".odbc_result($row, "Registration No_")."')  " ) or die(odbc_errormsg($conn));
