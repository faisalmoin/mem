	...
	<?php
		...
		for($f=0; $f<=$_REQUEST['fee_count']; $f++){
			...
			//Fee INSERT
					...
					VALUES('$inv_dt', '$inv_no', '$cust_no', '". odbc_result($rs, "Description")."', '".odbc_result($rs, "Amount")."', '-', 0, 0, '$LoginID', '$LoginID', '$ms', '', 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr',0)";
		...
		<td><?php echo number_format(odbc_result($rs, "Amount"),2,'.',','); ?></td>
		<td></td>
		<td></td>	
		</tr>
		<?php			
			
			$new_amt = odbc_result($rs, "Amount");
			...
			for($d=0; $d<=$_REQUEST['Dis_count']; $d++){
				$e=0;
					...
					if(odbc_result($disc_fee_line, "Description") != ""){
						...
						[Discount Code1],[Discount Code1 Amount], [Net Amount],[User ID], [Portal ID], [Company Name], [Discount Code2], [Discount Code2 Amount], [FinYr], [Month], [Year], [Qtr],[Reverse])
						VALUES('$inv_dt', '$inv_no', '$cust_no', '".odbc_result($disc_fee_line, "Description")."', 0, '".odbc_result($disc_fee_line, "Document No_")."', $e, 0, '$LoginID', '$LoginID', '$ms', '', 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr',0)";
		...
		[Discount Code1],[Discount Code1 Amount], [Net Amount],[User ID], [Portal ID], [Company Name], [Discount Code2], [Discount Code2 Amount], [FinYr], [Month], [Year], [Qtr],[Reverse])
		VALUES('$inv_dt', '$inv_no', '$cust_no', 'Net ".odbc_result($rs, "Description")." payable', 0, '', 0, $new_amt, '$LoginID', '$LoginID', '$ms', '', 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr',0)";
		...
		*/
		...
		// Net Amount INSERT
                $one_ins = "INSERT INTO [Ledger Invoice]([Invoice Date], [Invoice No], [Customer No], [Fee Description], [Amount],
					[Discount Code1],[Discount Code1 Amount], [Net Amount],[User ID], [Portal ID], [Company Name], [Discount Code2], [Discount Code2 Amount], [FinYr], [Month], [Year], [Qtr],[Reverse])
					VALUES('$inv_dt', '$inv_no', '$cust_no', 'Transport Fee', '$TransFee', '-', 0, 0, '$LoginID', '$LoginID', '$ms', '', 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr',0)";
					odbc_exec($conn, $one_ins) or exit(odbc_errormsg($conn)); // OneTime
		...
		$Net_payable = "INSERT INTO [Ledger Invoice]([Invoice Date], [Invoice No], [Customer No], [Fee Description], [Amount],
		[Discount Code1],[Discount Code1 Amount], [Net Amount],[User ID], [Portal ID], [Company Name], [Discount Code2], [Discount Code2 Amount], [FinYr], [Month], [Year], [Qtr],[Reverse])
		VALUES('$inv_dt', '$inv_no', '$cust_no', 'Net Transport Fee payable', 0, '', 0, $TransFee, '$LoginID', '$LoginID', '$ms', '', 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr',0)";
		odbc_exec($conn, $Net_payable) or exit(odbc_errormsg($conn)); // OneTime
...
<!--?php
  $EnqReg = odbc_prepare($conn, $InvCrd);
	            ...
	            exit("<div class='container'>
	                    <div class='bs-example'>
                            <div class='alert alert-danger alert-error'>
                                    ...
                                    <strong>Conformation</strong>Admission conform..<br />
                                    <br /><b>Invoice Number is .$inv_no...</b> ".odbc_errormsg($conn)."
                            </div>
	                    </div></div>");
	        }
...
?-->
<script>
       window.alert('Admission Confirm..Invoice Number is <?php echo $inv_no ?>')
        var childWindows = [];
