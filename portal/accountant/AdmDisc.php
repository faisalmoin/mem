...
<table id="results2" class="table table-responsive">
	...
	/*	
	  if($_REQUEST['EWS'] != 0){
	   $d=1;
		...
		$rs = odbc_exec($conn, "SELECT * FROM [Discount Fee Header] WHERE [Company Name]='$CompName' AND $where1 AND ([Class]='$Class' OR [Class]='') ORDER BY [Academic Year]");
	...
	<?php
		...
		$rs = odbc_exec($conn, "SELECT * FROM [Discount Fee Header] WHERE [Company Name]='$CompName' $where1 AND ([Class]='$Class' OR [Class]='') ORDER BY [Academic Year]");
		while(odbc_fetch_array($rs)){
...
// var total=document.querySelectorAll('input[name="discount[]"]:checked').length;
//alert(total);
</script>
