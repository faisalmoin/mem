<?php
    ...
    $invdatedetails = odbc_exec($conn, "SELECT * FROM [Ledger Invoice] WHERE [Company Name]='$ms' AND [Customer No]= '$CustomerNo' AND [Invoice No]= '$invoice' AND [Reverse]=0 ") or die(odbc_errormsg($conn));

   $fee_amt = odbc_exec($conn, "SELECT SUM([Credit Amount]) FROM [Ledger Credit] WHERE [Reverse]=0 AND [Company Name]='$ms' AND [Customer No]='$CustomerNo' AND [Invoice No]= '$invoice' ") or die(odbc_errormsg($conn));
...
<div class="title_left"><h1>Fee Reversal </h1>
</div>
...
<div class="x_panel">
<div class="x_title">
<h2>Student Details </h2> <!-- Page Name -->
<div class="clearfix"></div>
</div>
<div class="x_content">
<!-- Content -->
  <?php 
  $Admission = odbc_exec($conn, "SELECT * FROM [Temp Student] WHERE [Registration No_]='$CustomerNo' AND [Company Name]='$ms'") or die(odbc_errormsg($conn));
  ...
  <td style="border: none;">Fathers Name</td>
  <td style="border: none;"><span class="text-primary" style="font-weight: bold;"><?php echo strtoupper(odbc_result($Admission, "Father_s Name"))?></span></td>
  <td style="border: none;">Mothers Name</td>
  <td style="border: none;"><span class="text-primary" style="font-weight: bold;"><?php echo strtoupper(odbc_result($Admission, "Mother_s Name"))?></span></td>
  ...
  <td style="border: none;"><span class="text-primary" style="font-weight: bold;"><?php echo strtoupper(odbc_result($Admission, "Class"))?></span></td>
...
<div class="x_content">
          ...
          <a data-toggle="collapse" href="#collapse1">
            ...
            <table id="datatable" class="table table-responsive">
              <tr style="background-color: #ffffff;">
                ...
                <td style="border: none;"><span class="text-primary" style="font-weight: bold;"><?php echo $invoice;?></span></td>
                <td style="border: none;">Invoice Date</td>
                <td style="border: none;"><span class="text-primary" style="font-weight: bold;"><?php echo date('d/M/Y',odbc_result($invdatedetails, "Invoice Date"))?></span></td>
                <td style="border: none;">Invoice Amount</td>
                <td style="border: none;"><span class="text-primary" style="font-weight: bold;"><?php echo number_format(odbc_result($fee_amt, ""), 2, '.', '')?></span></td>
            </tr>
            </table>
          </a>
        </h4>
  ...
  <?php
  $sql1 = "SELECT * FROM [ledger invoice] WHERE [Company Name]='$ms' AND [Customer No]='$CustomerNo' AND [Invoice No]='$invoice' AND Reverse=0 "; 
...
<div class="x_content">
  ...
  <table id="datatable" class="table table-responsive" style="border: none;">
  <tr>
  <td style="border: none;"><span class="text-primary" style="font-weight: bold;">Action</span></td>
  <td style="border: none;"><span class="text-primary" style="font-weight: bold;">Remark</span></td>
  </tr>
  <tr>
  <td style="border: none;">
    <select class="form-control" name="Reverse">
      <option value="1">Reverse</option>
      <option value="2">Reverse And Generate</option>
    </select></td>
  <td style="border: none;"><input class="form-control" type="text" value="" name="Remark" required="true" /></td>
  </tr>
  </table>
