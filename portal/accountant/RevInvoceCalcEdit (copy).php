	...
	<?php
		...
		<?php			
			...
			for($d=0; $d<=$_REQUEST['Dis_count']; $d++){
				$e=0;
					...
					//echo "SELECT [No_] FROM [Discount Fee Header] WHERE [id]='".$_REQUEST['discount_Id'.$d]."' AND [Company Name]='$CompName'";
                                        //echo "SELECT [Description], [Discount%], [Document No_] FROM [Discount Fee Line] WHERE [Company Name]='$CompName' AND [Fee Code]='".odbc_result($rs, "Fee Code")."' AND [Academic Year]='$FinYr' AND [Document No_]='".odbc_result($disc_fee_hdr, "No_")."' ";
		...
		*/
		?>
            <!---- Transport Fee start------------------------------------------------------------------------------------------->
                <?php
              if(!empty($_REQUEST['TransFee']) && ($_REQUEST['TransFee']) && ($_REQUEST['TransDist'])) {
              $TransFee= $_REQUEST['TransFee'];
              $SlabCode= $_REQUEST['SlabCode'];
              $TransDist= $_REQUEST['TransDist'];
              if($_REQUEST['SlabCode'] != "" && $_REQUEST['TransFee'] != "" && $_REQUEST['TransDist'] != ""){
                  
             // update transport slap
                 $sql_update = "UPDATE [Temp Application] SET 
					[Slab Code]='".ucwords(strtoupper($_REQUEST['SlabCode']))."',
					[Transport Fee]='".$_REQUEST['TransFee']."',
					[Distance Covered in KM]='".$_REQUEST['TransDist']."'
					WHERE [Company Name]='$ms' AND [System Genrated No_]= '".$cust_no."' ";
		...
		echo "<td style='text-align:right;'>".number_format($TransFee,0,'.',',')."</td>";
		echo "</tr>";
		// Net Amount INSERT
                $one_ins = "INSERT INTO [Ledger Invoice]([Invoice Date], [Invoice No], [Customer No], [Fee Description], [Amount],
					[Discount Code1],[Discount Code1 Amount], [Net Amount],[User ID], [Portal ID], [Company Name], [Discount Code2], [Discount Code2 Amount], [FinYr], [Month], [Year], [Qtr])
					VALUES('$inv_dt', '$inv_no', '$cust_no', 'Transport Fee', '$TransFee', '-', 0, 0, '$LoginID', '$LoginID', '$ms', '', 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr')";
					//odbc_exec($conn, $one_ins) or exit(odbc_errormsg($conn)); // OneTime
		...
		$Net_payable = "INSERT INTO [Ledger Invoice]([Invoice Date], [Invoice No], [Customer No], [Fee Description], [Amount],
		[Discount Code1],[Discount Code1 Amount], [Net Amount],[User ID], [Portal ID], [Company Name], [Discount Code2], [Discount Code2 Amount], [FinYr], [Month], [Year], [Qtr])
		VALUES('$inv_dt', '$inv_no', '$cust_no', 'Net Transport Fee payable', 0, '', 0, $TransFee, '$LoginID', '$LoginID', '$ms', '', 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr')";
		//odbc_exec($conn, $Net_payable) or exit(odbc_errormsg($conn)); // OneTime
		...
		//odbc_exec($conn, $InvCrd) or exit(odbc_errormsg($conn));
                ...
                $chk_dr = odbc_exec($conn, "SELECT * FROM [Ledger Credit] WHERE 
						[Customer No] = '$cust_no'
                                                AND [Invoice No]='$InvoiceNo'
						AND [Company Name]='$ms' ")or exit(odbc_errormsg($conn));
                $Debitrel = "INSERT INTO [Ledger Debit]([Invoice Date], [Invoice No], [Customer No], Description, [Credit Amount], [Debit Amount],
				[Payment Date], [Payment Mode], [Bank Name], [Cheque No], [Cheque Date], [Payment Realization], [Company Name],
				[User ID], [Portal ID], Remarks, [Realization Date], [Posting No], [Adv Fee], [FinYr], [Month], [Year], [Qtr])
				VALUES(
				'".odbc_result($chk_dr, 'Invoice Date')."', '".odbc_result($chk_dr, 'Invoice No')."', '".$cust_no."',
				'".odbc_result($chk_dr, 'Description')."',  ".odbc_result($chk_dr, 'Credit Amount').", ".odbc_result($chk_dr, 'Credit Amount').",
				'$inv_dt', 'CASH', '', '', '', 1, '$ms', 
				'$LoginID', '$LoginID', 'Payment Receipt', '', $post1, 0 ,'$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr' )";
                //odbc_exec($conn, $Debitrel) or exit(odbc_errormsg($conn)); // OneTime
                //-----------------------------------------------------------------------
                 //-----------------Transport Fee  End------------------------------------------------------------------------------------------------
