...
<?php 
	...
	if($_REQUEST['PaymentMode'] !="CASH" && $_REQUEST['BankName']=="" && $_REQUEST['RegistrationCost'] !=0) exit("<div class='alert alert-danger alert-error'>
			                    <a href='#' class='close' data-dismiss='alert'>&times;</a>
			                    <strong>Error!</strong>You need to enter Bank Name ...</div>");
	if($_REQUEST['PaymentMode'] !="CASH" && $_REQUEST['ChequeDDNo']=="" && $_REQUEST['RegistrationCost'] !=0) exit("<div class='alert alert-danger alert-error'>
			                    <a href='#' class='close' data-dismiss='alert'>&times;</a>
			                    <strong>Error!</strong>You need to enter Cheque / DD Number ...</div>");
	if($_REQUEST['PaymentMode'] !="CASH" && $_REQUEST['ChequeDDDate']=="" && $_REQUEST['RegistrationCost'] !=0) exit("<div class='alert alert-danger alert-error'>
			                    <a href='#' class='close' data-dismiss='alert'>&times;</a>
        ...
        $AppNo= "WPR";
          ...
          if(odbc_result($ChkApp, "No_") == 0){
			...
			$InvCrd = "INSERT INTO [Ledger Credit]([Invoice Date], [Invoice No], [Customer No], Description,
				...
				VALUES 
				('$EpochTime', '$inv_no', '$SysGenNo', 'Sale of Registration', '$ApplicationCost', '$ms', '$LoginID', '$LoginID', $inv, 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr',0)";
                        if($_REQUEST['EWSStatus']==1 || $ApplicationCost==0){
				...
				[Payment Realization], [Company Name], [User ID], [Portal ID], Remarks, [Realization Date], [Posting No], [Adv Fee], [FinYr], [Month], [Year], [Qtr],[Reverse]) 
				VALUES 
				('$EpochTime', '$inv_no', '$SysGenNo', 'Sale of Registration', '$ApplicationCost','$ApplicationCost', '$EpochTime',
				'$PaymentMode', '$BankName', '$CheckNo', '$CheckDt', 1, '$ms', '$LoginID', '$LoginID', 'Subject to realization', '$EpochTime', $inv, 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr',0)";
			...
			$InvDbt = "INSERT INTO [Ledger Debit]([Invoice Date], [Invoice No], [Customer No], Description,
				...
				VALUES 
				('$EpochTime', '$inv_no', '$SysGenNo', 'Sale of Registration', '$ApplicationCost','$ApplicationCost', '$EpochTime',
				'$PaymentMode', '$BankName', '$CheckNo', '$CheckDt', 0, '$ms', '$LoginID', '$LoginID', 'Subject to realization', '', $inv, 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr',0)";
