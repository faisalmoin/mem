...
<!-- Content -->
       ...
       /* echo "<pre>";
        ...
        $d=1;
		$gTotal = 0;
        $FeeType = $_REQUEST['FeeType'.$f];
        $Reg= $_REQUEST['registration'];
        $Class= $_REQUEST['Class'];



   $FeeTpe = odbc_exec($conn, "SELECT * FROM [Fee Type] WHERE [Company Name]='$CompName' AND [Code]= '".$_REQUEST['FeeType'.$f]."' ") or die(odbc_errormsg($conn));
   ...
   if($today > $StartDate || $today > $EndDate){
       ...
       // window.location.href="Reverse.php?invoice='.$Reg.'";
       ?>
       </table>
           ...
           // echo "<br/><br/>";
            ...
            // echo $new_amt;
             ...
             $DocumentNo_= odbc_result($disc_fee_hdr, 'DocumentNo_');
                 ...
                 while(odbc_fetch_array($disc_fee_hdr)){
                   // echo odbc_result($disc_fee_hdr, 'DocumentNo_');
