	...
	<?php
		...
		*/
		?>
            <!---- Transport Fee start------------------------------------------------------------------------------------------->
                <?php
                $Transport = odbc_exec($conn, "SELECT * FROM [Temp Application] WHERE [Company Name]='$ms' AND [System Genrated No_]= '".$cust_no."' ") or die(odbc_errormsg($conn));
		...
		//odbc_exec($conn, $InvCrd) or exit(odbc_errormsg($conn));
                ...
                $chk = odbc_exec($conn, "SELECT * FROM [Ledger Debit] WHERE 
						[Customer No] = '$cust_no'
                                                AND [Invoice No]='$InvoiceNo'
						AND [Company Name]='$ms' ")or exit(odbc_errormsg($conn));
               if(odbc_num_rows($chk)==0){ 
                echo  "INSERT INTO [Ledger Debit]([Invoice Date], [Invoice No], [Customer No], Description, [Credit Amount], [Debit Amount],
				...
				'".odbc_result($chk_dr, 'Description')."',  ".odbc_result($chk_dr, 'Credit Amount').", ".odbc_result($chk_dr, 'Credit Amount').",
				'$inv_dt', 'CASH', '', '', '', 1, '$ms', 
				'$LoginID', '$LoginID', 'Rev Payment Receipt', '', $post1, 0 ,'$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr' )";
                                //odbc_exec($conn, $Debitrel) or exit(odbc_errormsg($conn)); // OneTime
                          }/*else {?> <div class="alert alert-warning">
                          <strong>Warning!</strong> This Invoice Number all ready Reverse.
                          </div>';<?php }*/
                //--------------------------------debit reverse entry---------------------------------------
                 //-----------------Transport Fee  End------------------------------------------------------------------------------------------------
