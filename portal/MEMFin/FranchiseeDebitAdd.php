...
<?php
    require_once("../db.txt");
    odbc_exec($conn, "INSERT INTO [MemFin Franchisee Debit]([Debit Amount], [Deduction Amount], [Total Amount], [Date], [Mode Of Payment],
			[Bank Name], [Cheque DD No], [Cheque DD Date],[Trust ID],[Trust Name],[Voucher Number]) 
    		...
    		'".strtotime(str_replace("/", " ",$_REQUEST["ChequeDDt"].date('H:i:s')))."', 
			 '".$_REQUEST['ID']."', '".$_REQUEST['TrustName']."', '".$_REQUEST['VoucherNo']."')") or die(odbc_errormsg($conn));
