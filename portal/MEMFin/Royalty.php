               ...
               <table class="table table-responsive " border="1" width="100%" id="abc" class="table table-fixed">
                     ...
                     <?php
                                ...
                                while(odbc_fetch_array($FeeHead1)){
                                        ...
                                        $Pay = odbc_exec($conn, "select sum([Net Amount]) from [Ledger Invoice] WHERE [FinYr]='$FinYr' AND [Reverse]=0 AND [Fee Description] LIKE 'Net ".ucwords(strtolower(odbc_result($Check_Fee1, "Fee Description")))." payable' AND [Customer No] IN (SELECT [System Genrated No_] FROM [Temp Application] WHERE [Company Name]='".odbc_result($rs, "ID")."' AND [Class]='".odbc_result($rs1, "Code")."') ");
                         ...
                         <?php
                                ...
                                while(odbc_fetch_array($FeeHead2)){
                                        ...
                                        if(odbc_num_rows($Check_Fee2) != 0){                                         
                                           $Pay1 = odbc_exec($conn, "select sum([Amount Paid]) from [Ledger Payment] WHERE [Reverse]=0 AND [Fee Description] LIKE 'Net ".ucwords(strtolower(odbc_result($Check_Fee2, "Fee Description")))." payable' AND [Customer No] IN (SELECT [System Genrated No_] FROM [Temp Application] WHERE [Company Name]='".odbc_result($rs, "ID")."' AND [Class]='".odbc_result($rs1, "Code")."') AND [Company Name]='$CompName'");
