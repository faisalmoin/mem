...
<html lang="en">
  <head>
	...
	<?php echo $SchName;?>
					...
					<ul class="nav navbar-nav">
						...
						<li class="dropdown">
							...
							<ul class="dropdown-menu">
							    <li><a href="PaymentRcvd.php">Payment Received</a></li>
								<li><a href="RoyaltyList.php">Royalty</a></li>
								<li><a href="FranchiseeList.php">Franchisee</a></li>
								<li><a href="RoyaltySetup.php?pg_id=1">Royalty SetUp</a></li>
							</ul>
						</li>
						<!--li class="dropdown">
		...
		<?php
                    //Num Schools
                    $num_sch = odbc_exec($conn, "SELECT [ID] FROM [Company Information] WHERE [ID] NOT IN (SELECT DISTINCT([Company Name]) FROM [Royalty Setup])");
                    $num1 = odbc_num_rows($num_sch);
                    while(odbc_fetch_array($num_sch)){
                        $sch .= odbc_result($num_sch, "ID").", ";
                    }
                    $sch = substr($sch, 0,-2);
			...
			<a href="SchoolList.php?sch='.$sch.'" class="text-danger"><strong>Info ! </strong> Missing Royalty information from <b>'.$num1.'</b> Schools ...</a>
			</div></div></div>';
                    }                     
