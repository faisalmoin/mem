<?php
	...
	{
		$Acad = odbc_exec($conn, "Delete FROM [Fee Components] WHERE [ID]='{$_REQUEST['delete_id']}'") or exit(odbc_errormsg($conn));
...
<table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
	...
	<?php
		$i = 1;
		$rs = odbc_exec($conn, "SELECT * FROM [Fee Components] WHERE [Company Name]='$CompName'");
		while(odbc_fetch_array($rs)){
	?>
	<tr>
		<td><?php echo $i; ?></td>
		<td><a href="FeeComponentEdit.php?ComponentId=<?php echo odbc_result($rs, "ID")?>"><?php echo odbc_result($rs, "Code")?></a></td>
...
function delete_id(ID)
{
     ...
     {
        window.location.href='FeeComponentList.php?delete_id='+ID;
     }
}
</script>
