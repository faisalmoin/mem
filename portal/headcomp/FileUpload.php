...
//if($_REQUEST["fileToUpload"] != ""){
	$target_dir = "../uploads/";
	$target_file = $target_dir . uniqid(). basename($_FILES["fileUpload".$i]["name"]);
	$uploadOk = 1;
	$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
	// Check if image file is a actual image or fake image
	if(isset($_POST["submit"])) {
		$check = getimagesize($_FILES["fileUpload".$i]["tmp_name"]);
		if($check !== false) {
	...
	// Check file size
	if ($_FILES["fileUpload".$i]["size"] > 500000) {
	...
	// if everything is ok, try to upload file
		...
		if (move_uploaded_file($_FILES["fileUpload".$i]["tmp_name"], $target_file)) {
