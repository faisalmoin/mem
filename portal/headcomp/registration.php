<?php
  ...
  //  $rn = "RN".$cid.date('ym');//echo $rn;    
   ...
   // $rn .= str_pad(odbc_result($RegistrationNo, "C"), 3, "0", STR_PAD_LEFT); //Registration No. 
    ...
    $SysGenNo = "ENQ".str_pad(odbc_result($SysNo, ""), 3, '0', STR_PAD_LEFT);
   ...
   // $RegNo = strtoupper($rn);
	...
	$stmt = odbc_prepare($conn, $MDBSQL) or die(odbc_errormsg($conn));
	...
	$body .= "<p align='justify'><a href='http://202.54.232.182/registration/PrintForm.php?cid=$cid&RegNo=$SysGenNo' style='padding: 10px; border: 1px solid #8800ff;background-color: #4961e1; color: #ffffff;'>Print Form</a>";
	...
	?>
		<div class='container'>
			...
			<?php 
				...
				echo  "<p align='justify'>The application no is <b>$SysGenNo</b>. Please produce the mentioned application no. whenever asked by the school authorities at the time of verification & registration.</p>";
	...
	of the date of visit to school for physical submission of form shortly.</p>";
			...
			?>
			<br /><a href='PrintForm.php?cid=<?php echo $cid?>&RegNo=<?php echo $SysGenNo?>&No=<?php echo md5($security)?>' style="padding: 10px; border: 1px solid #8800ff;background-color: #4961e1; color: #ffffff;">Print Form</a>
			</div>
	<?php
	    }
	    else{
