...
require_once("../db.txt");
    ...
    $cid = '6';
     ...
     for($d=0; $d<=15; $d++){
     //Check the values in the field.
     $checkField = odbc_exec($conn, "select * from [FieldType] where [CompanyName]='".$cid."' AND [FieldName]='".$_REQUEST['check'.$d]."' ") or exit(odbc_errormsg($conn));
     if(odbc_num_rows($checkField)==0){
     //if value not available, INSERT
     if($_REQUEST['check'.$d] != ""){
     $SQL2 = "INSERT INTO [FieldType] ([FieldName], [CompanyName], [CheckStatus]) VALUES ('".$_REQUEST['check'.$d]."','".$cid."','".$_REQUEST['text'.$d]."')";
     echo "SQL2: ".$SQL2;
     $result2 = odbc_exec($conn, $SQL2) or exit(odbc_errormsg($conn));
     }
     //if value available in table, but user did not required, then DELETE the field.
     }else{
    if($_REQUEST['check'.$d] == "" && $_REQUEST['text'.$d] != ""){
    // if($_REQUEST['check'.$d] != checked){
     $SQL3 = "Delete FROM [FieldType] where [CompanyName]='".$cid."' AND [FieldName]='".$_REQUEST['check'.$d]."' ";
     echo "SQL3: ".$SQL3;
     $result3 = odbc_exec($conn, $SQL3) or exit(odbc_errormsg($conn));
     }
     }
     }
	...
	/*
	for($d=0; $d<=15; $d++){
		//if($_REQUEST['check'.$d] == 0){
		echo $_REQUEST['check'.$d]."<br />";
		//}
	}*/
    ...
    for($d=0; $d<=14; $d++){
    	//Check the values in the field.
	echo $_REQUEST['check'.$d]." _ ";
	echo "select * from [FieldType] where [CompanyName]='".$cid."' AND [FieldName]='".$_REQUEST['check'.$d]."' <br />";
    	$checkField = odbc_exec($conn, "select * from [FieldType] where [CompanyName]='".$cid."' AND [FieldName]='".$_REQUEST['check'.$d]."' ") or exit(odbc_errormsg($conn));
    	if(odbc_num_rows($checkField)==0){
    		//if value not available, INSERT
    		if($_REQUEST['check'.$d] != ""){
    			$SQL2 = "INSERT INTO [FieldType] ([FieldName], [CompanyName]) VALUES ('".$_REQUEST['check'.$d]."','".$cid."')";
    			echo "SQL1: ".$SQL2."<br />";
    			//$result2 = odbc_exec($conn, $SQL2) or exit(odbc_errormsg($conn));
    		}
    		//if value available in table, but user did not required, then DELETE the field.
  ...
  //echo "SELECT * FROM [Company Information] WHERE [ID]='$cid' ";
  while(odbc_fetch_array($Comp))
				
  //  $Company = odbc_exec("SELECT * FROM [Company Information] WHERE id= '$cid' ") or die(odbc_error());
  ...
  /*  $rn = "RN".date('ym');
    ...
    $RegistrationNo = odbc_exec("SELECT COUNT(`id`)+1 FROM `enquiry` WHERE `EnquiryNo` LIKE '$rn' ") or die(odbc_error());
    $rno = odbc_fetch_array($RegistrationNo);

    $rn .= str_pad($rno[0], 8, '0', STR_PAD_LEFT); //Registration No.
*/
?>
	<!-- Bootstrap core CSS -->
...
<div class="container">
   <form name="myForm" method="POST" action="CompanyReg.php" onsubmit="return(validateForm());" onkeypress="return event.keyCode != 13;" enctype="multipart/form-data">
...
<table width="100%" align="center" border="0px" cellspacing="0px" cellpadding="5px"  class="table table-responsive borderless"  style="font-family: 'Oxygen', Arial; font-size: 18px;">
    ...
    <tr>
        ...
        <td style="border-top: 0px;">
            <input type='checkbox' value='1' name='Sibling'> Sibling in School<br />
            <input type='checkbox' value='1' name='Staff'> Staff Child
        </td>
    </tr> -->
    <tr>
 ...
 $check = odbc_exec($conn, "select * from [FieldType] where [CompanyName]='".$cid."' ") or exit(odbc_errormsg($conn));
 while(odbc_fetch_array($check))
 // echo "select * from [FieldType] where [CompanyName]='".$cid."'";
 // echo odbc_result($check, "FieldName");
    if(odbc_result($check, "FieldName")== "ChildName") {?>
    <tr>
        <td>1</td>
    ...
    <?php }?>
    <?php if(odbc_result($check, "FieldName")== "Nationality") {?>
    ...
    <?php }?>
    <?php if(odbc_result($check, "FieldName")== "DOB") {?>
   <tr>
        <td>3</td>
    ...
    <?php }?>
	<?php if(odbc_result($check, "FieldName")== "MotherTongue") {?>
	<tr>
		<td>4</td>
		<td>Mother Tongue</td>
	...
	<?php }?>
	<?php if(odbc_result($check, "FieldName")== "ReligionCaste") {?>
	<tr>
		<td>5</td>
		<td>Religion <sup style="color: #990000; font-weight: bold">*</sup> 
	...
	<?php }?>
	<?php if(odbc_result($check, "FieldName")== "ClassAdmission") {?>
    ...
    <tr>
		...
		<select name="ClassApplied" class="form-control input" id="input">
    ...
    <?php }?>
    <?php if(odbc_result($check, "FieldName")== "ParentInformation") {?>
    ...
    <?php }?>
    <?php if(odbc_result($check, "FieldName")== "ResidentialPhone") {?>
    <tr>
        <td>9</td>
    ...
    <?php }?>
    <?php if(odbc_result($check, "FieldName")== "EmergencyPhone") {?>
   <tr>
        <td>10</td>
    ...
    <?php }?>
    <?php if(odbc_result($check, "FieldName")== "Email") {?>
    <tr>
        <td></td>
    ...
    <?php }?>
   <?php if(odbc_result($check, "FieldName")== "MaritalStatus") {?>
   <tr>
        <td>11</td>
    ...
    <?php }?>
    <?php if(odbc_result($check, "FieldName")== "Distance") {?>
   <tr>
        <td>12</td>
        <td>
    ...
    <?php }?>
     <?php if(odbc_result($check, "FieldName")== "major") {?>
	<tr>
		<td>13</td>
		<td>
	...
	<?php }?>
	<?php if(odbc_result($check, "FieldName")== "SiblingSchool") {?>
    <tr>
        <td>14.</td>
        <td>
            a. Sibling in School
