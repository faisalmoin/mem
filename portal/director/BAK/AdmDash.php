		...
		$body .="<table width='100%'>
				...
				$body .=	"<td style='background: #D8F6CE' align='center'>Total Admission</td>
					...
					<td style='background: #F5D0A9' align='center'>Total Student Strength</td>					          
						...
						$i=1;
						//Distinct Company
					        $Comp = odbc_exec($conn, "SELECT [ID] AS [Company Name] FROM [Company Information] WHERE [Company Status]=1 ORDER BY [ID] ASC") or exit("Line 139");
				...
				<tr style='font-weight: bold;'>
					...
					<td align='center' style='background: #D8F6CE'>";
						$totalEnquiry = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Enquiry] WHERE [Admission For Year]='$ac1' AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1)  ") or exit("Line 253");
						$body .= odbc_result($totalEnquiry, "");						
				$body .= "</td>
					<td align='center' style='background: #D8F6CE'>";
						$TotReg = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Application] WHERE [Admission For Year] ='$ac1' AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1) ") or exit("Line 257");
						$body .= odbc_result($TotReg, "");
				$body .= "</td>
					<td align='center' style='background: #D8F6CE'>";
						$TotAdm = odbc_exec($conn, "SELECT COUNT([No_]) From [Temp Student] WHERE [Admission for Year]='$ac1' AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1)    ") or exit("Line 261");
						$body .= round(odbc_result($TotAdm, ""),0);
				$body .= "</td>
					<td align='center' style='background: #D8F6CE'>";
						$TotInactive_1a = odbc_exec($conn, "SELECT COUNT([No_]) From [Temp Student] WHERE ([Date of Leaving] BETWEEN '".$dl1."-01-01' AND '".$dl1."-12-31') AND [Student Status]=2 AND [Admission For Year] = '$ac2' AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1)   ") or exit("Line 265");
						$TotInactive_2a = odbc_exec($conn, "SELECT COUNT([No_]) From [Temp Student] WHERE ([Date of Leaving] BETWEEN '".$dl1."-01-01' AND '".$dl1."-12-31') AND [Student Status]=2 AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1)   ") or exit("Line 262");
				...
				$body .= "</td>
					<td align='center' style='background: #D8F6CE'>";
						$TotTC_1a = odbc_exec($conn, "SELECT COUNT([No_]) From [Temp Student] WHERE ([Date of Leaving] BETWEEN '".$dl1."-01-01' AND '".$dl1."-12-31') AND [Student Status]=3 AND [Admission For Year] = '$ac2' AND [Company Name] IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1) ") or exit("Line 273");
						$TotTC_2a = odbc_exec($conn, "SELECT COUNT([No_]) From [Temp Student] WHERE ([Date of Leaving] BETWEEN '".$dl1."-01-01' AND '".$dl1."-12-31') AND [Student Status]=3 AND [Company Name] IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1) ") or exit("Line 274");
				...
				$body .= "</td>
					...
					if(strtotime(date('Y-m-d')) > strtotime(odbc_result($End_date, "End Date"))){
						$TotStu1 = odbc_exec($conn, "SELECT count([No_]) FROM [Temp Student] WHERE [Student Status]=1 AND [Admission For Year] <> '$ac2' AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1) ") or die("Line 290");
						$TotStu2 = odbc_exec($conn, "SELECT count([No_]) FROM [Temp Student] WHERE [Student Status]<>1 AND [Admission For Year] <> '$ac2' AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1)  AND [Date of Leaving] BETWEEN '01/Jan/".date('Y')."' AND '31/Dec/".date('Y')."' ") or die("Line 291");
						$TotStu3 = odbc_exec($conn, "SELECT count([No_]) FROM [Temp Student] WHERE [Student Status]<>1 AND [Admission For Year] = '$ac2' AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1)  AND [Date of Leaving] BETWEEN '01/Jan/". (date('Y')-1)."' AND '31/Dec/".(date('Y')-1)."' ") or die("Line 292");
						$TotStu = (odbc_result($TotStu1, "")+odbc_result($TotStu2, "")-odbc_result($TotStu3, ""));
						
						$body .= round($TotStu,0);
					}
					else{
						$TotStu = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Student] WHERE [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1 )  AND [Academic Year]='$ac1' AND [Student Status]=1  ") or exit("Line 298");
						$body .= round(odbc_result($TotStu, ""),0);
					  }		
				$body .= "</td>
					...
					<td style='background: #F5D0A9' align='center'>";
						$totalEnquiry1 = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Enquiry] WHERE  [Admission For Year]='$ac2' AND [Company Name] IN (SELECT [ID] FROM [Company Information] WHERE [Company Status]=1 ) ") or exit("Line 305");
						$body .= odbc_result($totalEnquiry1, "");						
				$body .= "</td>
					<td style='background: #F5D0A9' align='center'>";
						$TotReg1 = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Application] WHERE [Admission For Year] ='$ac2' AND [Company Name] IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1 )  ") or exit("Line 309");
						$body .= odbc_result($TotReg1, "");
				$body .= "</td>
					<td style='background: #F5D0A9' align='center'>";
						$TotAdm1 = odbc_exec($conn, "SELECT COUNT([No_]) From [Temp Student] WHERE [Admission for Year]='$ac2' AND [Company Name] IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1 )    ") or exit("Line 313");
						$body .= round(odbc_result($TotAdm1, ""),0);
				$body .= "</td>
					<td style='background: #F5D0A9' align='center'>";
						$TotInactive1 = odbc_exec($conn, "SELECT COUNT([No_]) From [Temp Student] WHERE ([Date of Leaving] BETWEEN '".$dl2."-01-01' AND '".$dl2."-12-31') AND [Student Status]=2  AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1 )   ") or exit("Line 317");
						$body .= round(odbc_result($TotInactive1, ""),0);
				$body .= "</td>
					<td style='background: #F5D0A9' align='center'>";
						$TotTC1 = odbc_exec($conn, "SELECT COUNT([No_]) From [Temp Student] WHERE ([Date of Leaving] BETWEEN '".$dl2."-01-01' AND '".$dl2."-12-31') AND [Student Status]=3 AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1)    ") or exit("Line 321");
						$body .= round(odbc_result($TotTC1, ""),0);
				$body .= "</td>
					<td align='center' style='background: #F5D0A9'>";
						$TotWD1 = odbc_exec($conn, "SELECT COUNT([No_]) From [Temp Student] WHERE ([Date of Leaving] BETWEEN '".$dl2."-01-01' AND '".$dl2."-12-31') AND ([Student Status]=2 OR [Student Status]=3) AND [Company Name]  IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1)  ") or exit("Line 325");
						$body .= round(odbc_result($TotWD1, ""),0);
				$body .= "</td>
					<td style='background: #F5D0A9' align='center'>";
						$body .= (round(odbc_result($TotAdm1, ""),0)- ((round(odbc_result($TotTC1, ""),0) + round(odbc_result($TotInactive1, ""),0))));
				$body .= "</td>
					<td style='background: #F5D0A9' align='center'>";
						$TotStu1 = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Student] WHERE  [Student Status]=1 AND [Company Name] IN (SELECT [ID]  FROM [Company Information] WHERE [Company Status]=1)  ") or exit("Line 332");
						$body .= round(odbc_result($TotStu1, ""),0);
				$body .= "</td>
				</tr>
			</table> </td></tr></table>
