...
<h2 class="text-primary" align="center">Admission</h2>
...
$StuSecCount = odbc_exec($conn, "SELECT DISTINCT([Section]) FROM [Temp Student] WHERE [Company Name]='".$_GET['c']."' ORDER BY [Section]") or die(odbc_errormsg($conn));
	...
	//$CountStu = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Student]  WHERE [Company Name]='".$_GET['c']."'  AND [Class] = '".odbc_result($StuClass, 'Code')."' AND [Section] = '".odbc_result($StuSecCount, 'Section')."' AND [Admission For Year] = '".$_GET['y']."'  AND [Student Status] = 1 ");
	$CountStu = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Student]  WHERE [Company Name]='".$_GET['c']."'  AND [Class] = '".odbc_result($StuClass, 'Code')."' AND [Section] = '".odbc_result($StuSecCount, 'Section')."' AND [Admission For Year] = '".$_GET['y']."' ");
		if(odbc_result($CountStu, '') != 0){
			echo "<td align='center'>";
			echo "<a href='RptStudentStrength.php?cmp=$cmp&y=$AcadYr&c=".odbc_result($StuClass, 'Code')."&s=".odbc_result($StuSecCount, 'Section')." '>".odbc_result($CountStu, '')."</a>";
		}
		else{
	...
	//$ClassStrength = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Student] WHERE [Company Name]='".$_GET['c']."' AND [Class] = '".odbc_result($StuClass, 'Code')."' AND [Admission For Year] = '".$_GET['y']."'  AND [Student Status] = 1 ");
	$ClassStrength = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Student] WHERE [Company Name]='".$_GET['c']."' AND [Class] = '".odbc_result($StuClass, 'Code')."' AND [Admission For Year] = '".$_GET['y']."'  ");
...
<tr style="font-size: 18px;">
	...
	<td style="color: #000000;" align="center"><b>
		<?php
			//$SchStrength = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Student] WHERE [Company Name]='".$_GET['c']."'  AND [Admission For Year] = '".$_GET['y']."'  AND [Student Status] = 1 ");
			$SchStrength = odbc_exec($conn, "SELECT COUNT([No_]) FROM [Temp Student] WHERE [Company Name]='".$_GET['c']."'  AND [Admission For Year] = '".$_GET['y']."'  ");
