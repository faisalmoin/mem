...
<div class="container" style="overflow-x: hidden;">
	<h1 class="text-primary">Target List</h1>
	<div class="table-responsive">
		<table class="table table-striped table-hover  sticky-header">
			<form action="directorRptEnquiry1.php" name="form" method="get">
				...
				<td colspan="12" align="right"> Go to page
					<select name="ePage" onchange="this.form.submit()">						
						<?php
							...
							$enqCount=odbc_exec($conn, "SELECT (COUNT([No_])/50)+1 FROM [Temp Enquiry] WHERE [Company Name]='". $_GET['c']."' AND [Admission For Year]='". $_GET['y']."'");
			...
			<tbody>
				<?php
					...
					$result=odbc_exec($conn, "SELECT *   FROM ( 
							SELECT *, ROW_NUMBER() OVER (ORDER BY [Enquiry Date] DESC) as row FROM [Temp Enquiry] WHERE [Company Name]='".$_GET['c']."' AND [Admission For Year]='".$_GET['y']."'
