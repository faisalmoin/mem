...
<table id="results2" class="table table-responsive">
	...
	/*
	  ...
	  if($_REQUEST['EWS'] != 0){
	   $d=1;
		$rs = odbc_exec($conn, "SELECT * FROM [Discount Fee Header] WHERE [Company Name]='$CompName' AND [Fee Clasification Code]='RTE' $where1 ORDER BY [Academic Year]");
	...
	<tr>
		...
		<td style="text-align: center;"><?php echo odbc_result($rs, 'Class');?></td>
	...
	<?php
		...
		$rs = odbc_exec($conn, "SELECT * FROM [Discount Fee Header] WHERE [Company Name]='$CompName' $where1 AND [Fee Clasification Code] <> 'RTE' ORDER BY [Academic Year]");
		while(odbc_fetch_array($rs)){
