...
/*
	...
	Columns: Dept, Team, Name, Date, Arrive, Start, Break Out/In, Transfer Out/In, Stop, Depart, Location, Flags
...
class Report extends ReportBase {
	...
	public function initialise() {
		...
		for($cnt=0; $cnt<count($this->emps);$cnt++) {
			...
			$this->loadRawAttendance($cnt, $this->days[0], $this->days[count($this->days)-1] );
		...
		foreach ($this->emps as $emp) {
			...
			foreach ($this->days as $day) {
				...
				if ($day == $emp['att'][$acnt]['att_date']) {
					do {
//					$this->error->debug('Day: '.$day.', Att.Date: '.$emp['att'][$acnt]['att_date'].'<br>');
...
//	Columns: PayID, Dept, Team, Name, Date, Start, Type, Stop, Type, Location, Flags
						$this->data[$cnt++] = array($emp['payroll'], $emp['dep'], $emp['team'], $emp['name'], $day,
															 $emp['att'][$acnt]['start_type'],
															 $emp['att'][$acnt]['start_time'],
															 $emp['att'][$acnt]['end_type'],
															 $emp['att'][$acnt]['end_time'],
															 $emp['att'][$acnt]['name'],$flags);
						$acnt++;
					} while ($day == $emp['att'][$acnt]['att_date']);
				} else {
					if ($day > $this->today) {
					...
					$this->data[$cnt++] = array($emp['payroll'], $emp['dep'], $emp['team'], $emp['name'], $day,
														 "-","--:--","-","--:--",
														 " ", $flags);
				}			// end if attention data is present (or not)
			}				// end for each day
							// if there is no attention data at all AND we should not show empty datasets, then we
							// need to print a line saying "no data"
	...
	public function write2HTML() {
		...
		$out = "<table style=\"width: 100%;\">".$CRLF;
