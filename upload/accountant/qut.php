		...
		$Admission = odbc_exec($conn, "SELECT [No_] FROM [Temp Student] WHERE [Registration No_]='".$Reg."' AND [Company Name]='$CompName'") or die(odbc_errormsg($conn));
	       ...
	       <div class="container">
                    ...
                    <h2 class="text-primary"><?php echo $f." - ".$AdmissionNo ?></h2>
                    ...
                    function MonthNo($Mnth,$Reg){
                      ...
                      //  odbc_exec($conn, $one_ins) or exit(odbc_errormsg($conn)); // OneTime
                        ...
                        //-----------------------------------------start transport fee----------------------------------------------
                           ...
                           [Fee Description]='Transport Fee' AND [Customer No]='".$Reg."' AND [Year]='".date('Y', $today)."' AND [Month] = '".date('m', $today)."' AND [Qtr] = '$Qtr' ") or die(odbc_errormsg($conn)) ;
                           if(odbc_num_rows($checkMonth1)==0){
                           $one_ins = "INSERT INTO [Ledger Invoice]([Invoice Date], [Invoice No], [Customer No], [Fee Description], [Amount],
                           [Discount Code1],[Discount Code1 Amount], [Net Amount],[User ID], [Portal ID], [Company Name], [Discount Code2], [Discount Code2 Amount], [FinYr], [Month], [Year], [Qtr])
                           VALUES('$inv_dt', '$inv_no', '$cust_no', 'Transport Fee', '$a', '-', 0, 0, '$LoginID', '$LoginID', '$CompName', '', 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr')";
                           // odbc_exec($conn, $one_ins) or exit(odbc_errormsg($conn)); // OneTime
                            ...
                            $Net_payable = "INSERT INTO [Ledger Invoice]([Invoice Date], [Invoice No], [Customer No], [Fee Description], [Amount],
                            [Discount Code1],[Discount Code1 Amount], [Net Amount],[User ID], [Portal ID], [Company Name], [Discount Code2], [Discount Code2 Amount], [FinYr], [Month], [Year], [Qtr])
                            VALUES('$inv_dt', '$inv_no', '$cust_no', 'Net Transport Fee payable', 0, '', 0, $a, '$LoginID', '$LoginID', '$CompName', '', 0, '$FinYr', '".date('m', $today)."', '".date('Y', $today)."', '$Qtr')";
                            //odbc_exec($conn, $Net_payable) or exit(odbc_errormsg($conn)); // OneTime
                            }
                            }
                            }
